...
#     queryset = Land.objects.filter(ubigeo__in=['240303','120421','060905','021401','020108','240104','050405','250401','240105','220303','061201','170301','190206','040515','050911','010523','020601','130201','040701','220705','060105','140102','040703','100507']).values('id','ubigeo_id','cup')
...
def run():
    excel_file = path.join(settings.MEDIA_ROOT , 'cf_predio.xls')

    df = pd.read_excel(excel_file, dtype={'UBIGEO':str,'COD_SECT': str,'COD_MZN':str,'COD_UU':str,'COD_LOTE':str,'TIPO_UU':str,'TIP_VIA':str,'SEC_EJEC':str})

    queryset = Land.objects.filter(ubigeo__in=['220901']).values('id','ubigeo_id','cup')
    df_django = pd.DataFrame(list(queryset))
